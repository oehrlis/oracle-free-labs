# OraDBA Encryption - TDE Configuration (SQL Scripts)

This folder contains the configuration for the **odbenc** service - creates an
...
- **./config/odbenc/setup → /opt/oracle/scripts/setup**  
...
- **./config/odbenc/startup → /opt/oracle/scripts/startup**  
...
- Setup scripts should be numbered (for example `00_config_db.sql`,
  `10_create_pdb_odbenc.sql`) to enforce execution order.
