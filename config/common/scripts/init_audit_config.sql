...
--  Notes.....: Must be executed as SYSDBA connected to the PDB context
...
-- Parameters ------------------------------------------------------------------
-- &1 => Audit tablespace name (optional, default: AUDIT_DATA)
-- &2 => Tablespace size (optional, default: 20480K)
-- &3 => Audit retention (optional, default: 30)
...
DEFINE _TS_NAME = &1 AUDIT_DATA
DEFINE _TS_SIZE = &2 20480K
DEFINE _AUDIT_RETENTION = &3 30
SET FEEDBACK ON
...
DECLARE
  ...
  l_audit_tablespace  v$tablespace.name%TYPE := UPPER('&&_TS_NAME');
  ...
  BEGIN
    IF l_file_dest IS NOT NULL THEN
      ...
      l_sql := 'CREATE BIGFILE TABLESPACE ' || l_audit_tablespace ||
               ' DATAFILE SIZE &&_TS_SIZE AUTOEXTEND ON NEXT 10240K MAXSIZE UNLIMITED';
    ...
    ELSE
      ...
      IF l_datafile_path LIKE '+%' THEN
        ...
        l_sql := 'CREATE BIGFILE TABLESPACE ' || l_audit_tablespace ||
                 ' DATAFILE ''' || l_datafile_path || ''' SIZE &&_TS_SIZE AUTOEXTEND ON NEXT 10240K MAXSIZE UNLIMITED';
      ...
      ELSE
        ...
        l_sql := 'CREATE BIGFILE TABLESPACE ' || l_audit_tablespace ||
                 ' DATAFILE ''' || l_audit_data_file || ''' SIZE &&_TS_SIZE AUTOEXTEND ON NEXT 10240K MAXSIZE UNLIMITED';
  ...
  BEGIN
    sys.dbms_scheduler.create_job (
      ...
      job_action => 'BEGIN sys.dbms_audit_mgmt.set_last_archive_timestamp(audit_trail_type => 
                      sys.dbms_audit_mgmt.audit_trail_unified,last_archive_time => sysdate-&&_AUDIT_RETENTION); END;',
      start_date => sysdate,
      repeat_interval => 'FREQ=HOURLY;INTERVAL=24',
      enabled    =>  TRUE,
      comments   => 'Archive timestamp for unified audit to sysdate-&&_AUDIT_RETENTION'
    );
